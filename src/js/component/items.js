                ...
                {items.map((item, index) => (
                    <div className="col-12 col-md-6 col-lg-3" key={index}>
                        <div className="card card-style h-100">
                            <img className="card-img-top custom-image" src={item.image || placeholderImage} alt={item.name}></img>
                            ...
                            <div className="card-footer d-flex justify-content-between">
                                <Link to={`/details/${item.id}`} className="btn card-button">
                                    Learn More!
                                </Link>
                                <button
                                    onClick={() => actions.toggleFavorites(item.id, !store.favorites.includes(item.id))}
                                    className={store.favorites.includes(item.id) ? 'btn favorites-button-active' : 'btn card-button-outline heart-button'}
                                >
