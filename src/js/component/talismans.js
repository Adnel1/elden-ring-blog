                ...
                {talismans.map((talisman, index) => (
                    <div className="col-12 col-md-6 col-lg-3" key={index}>
                        <div className="card card-style h-100">
                            <img className="card-img-top custom-image" src={talisman.image || placeholderImage} alt={talisman.name}></img>
                            <div className="card-body d-flex flex-column">
                                <h6 className="card-title">{talisman.name}</h6>
                                <p
                            ...
                            <div className="card-footer d-flex justify-content-between">
                                <Link to={`/details/${talisman.id}`} className="btn card-button">
                                    Learn More!
                                </Link>
                                <button
                                    onClick={() => actions.toggleFavorites(talisman.id, !store.favorites.includes(talisman.id))}
                                    className={store.favorites.includes(talisman.id) ? 'btn favorites-button-active' : 'btn card-button-outline heart-button'}
                                >
