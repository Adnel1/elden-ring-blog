                ...
                {ammos.map((ammo, index) => (
                    <div className="col-12 col-md-6 col-lg-3" key={index}>
                        <div className="card card-style h-100">
                            <img className="card-img-top custom-image" src={ammo.image || placeholderImage} alt={ammo.name}></img>
                            <div className="card-body d-flex flex-column">
                                <h6 className="card-title">{ammo.name}</h6>
                                <p
                            ...
                            <div className="card-footer d-flex justify-content-between">
                                <Link to={`/details/${ammo.id}`} className="btn card-button">
                                    Learn More!
                                </Link>
                                <button
                                    onClick={() => actions.toggleFavorites(ammo.id, !store.favorites.includes(ammo.id))}
                                    className={store.favorites.includes(ammo.id) ? 'btn favorites-button-active' : 'btn card-button-outline heart-button'}
                                >
